...
# It's crucial here to use the correct reader based on the file extension.
# Since json_to_vtk.py now outputs .vtp, we'll prioritize that.
if TURBINE_MODEL_PATH.lower().endswith('.vtp'):
...
elif TURBINE_MODEL_PATH.lower().endswith('.obj'):
    turbine_reader = pv_s.OBJReader(FileNames=[TURBINE_MODEL_PATH])
else:
    print(f"Error: Unsupported turbine model format: {os.path.splitext(TURBINE_MODEL_PATH)[1]}. Supported: .vtp, .stl, .obj")
    sys.exit(1)
...
# Apply a transform to the line source to move it slightly upstream if needed
# transform_line = pv_s.Transform(Input=line_source)
# transform_line.Transform = 'Transform' # This creates a vtkTransform object
# transform_line.Transform.Translate = [-0.1, 0, 0] # Move 0.1 units in -X direction

streamlines = pv_s.StreamTracer(Input=fluid_reader, SeedSource=line_source) # Use line_source directly or transformed_line
